    ...
    def initNetwork(self):
        for name, param in self.named_parameters():
            if param.requires_grad:
                # print(name)
                if "weight" in name:
                    print("Parameter {:} initialized with Xavier".format(name))
                    torch.nn.init.xavier_uniform_(param.data)
                elif 'bias' in name:
                    print("Parameter {:} initialized with Uniform".format(name))
...
criterion = nn.CrossEntropyLoss() # WITHOUT TRANSFORM LABELS
